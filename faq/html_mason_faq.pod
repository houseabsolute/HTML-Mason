...
group, write to the HTML::Mason authors: Jonathan Swartz
(swartz@transbay.net).
...
offline or as a general templating tool.  See "standalone mode"
in the interpreter manual (Interp.pm).
...
=head2 I'm getting 404 errors for pages that clearly exist.
...
Use $m->abort, documented in the Request manual (Request.pm).
...
"Sending HTTP Headers" section in the component developers manual
(Devel.pod).
...
=head2 How can I capture the output of a component (and modify it,
etc.) instead of having it automatically output?

Use $m->scomp. See the $m->scomp section of the Request manual (Request.pm).
...
Mason Administrators guide (Admin.pod).

=head2 Do data cache files expire automatically when a component or its dependencies 
change?
...
buffered.  print and $r->print circumvent the buffer and thus come out
before other Mason output.

Solution: don't use print or $r->print. Use $m->out if you must output
inside a Perl section. See the section on output mode:

   http://www.masonhq.com/docs/manual/Admin.html#out_mode

and $m->out:

   http://www.masonhq.com/docs/manual/Commands.html#item_out_string_
