...
Finally, consider using a separate component, if the subroutine is large
enough, instead of a subroutine; they are functionally equivalent, and this
is playing to Mason's strengths.  If the subroutine is small, or limited in
scope, consider using <%def> subcomponents.
...
Please note that running HTML::Mason under CGI (or other non-persistant
...
modules for every CGI execution.  Using mod_perl or similar persistant
