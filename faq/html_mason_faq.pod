...
You should not define traditional named subroutines in components, e.g. "sub
foo {...}", because components are implemented as subroutines, and Perl does
not like named subroutines to be defined inside other subroutines.  It may
not raise an error, but it will not work reliably.  Except in odd situations,
you should use components to break your code into "subroutines" and use
mc_comp() to call them.

You can define anonymous subroutines inside components:
...
component arguments just as if you had called the component with mc_comp and
parameters. So you can get at GET/POST data by pre-declaring argument names
and/or using the %ARGS hash which is always available.
