...

{
    ...
    __PACKAGE__->valid_params(
        foo_val => { parse => 'string', type => Params::Validate::SCALAR } );
...

{
    ...
    __PACKAGE__->valid_params(
        bar_val => { parse => 'string', type => Params::Validate::SCALAR } );
...

{
    ...
    @HTML::Mason::Compiler::ToObject::Test::ISA =
      'HTML::Mason::Compiler::ToObject';

    __PACKAGE__->valid_params(
        baz_val => { parse => 'string', type => Params::Validate::SCALAR } );
...
sub make_tests {
    ...
    $group->add_test(
        name          => 'request_subclass',
        description   => 'use a HTML::Mason::Request subclass',
        interp_params => {
            request_class => 'HTML::Mason::Request::Test',
            foo_val       => 77
        },
        component => <<'EOF',
% if ( $m->can('foo_val') ) {
...
EOF
    ...
    $group->add_test(
        name          => 'request_subclass_of_subclass',
...
EOF
    ...
    $group->add_test(
        ...
        interp_params => {
            lexer_class => 'HTML::Mason::Lexer::Test',
            bar_val     => 76
        },
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name          => 'compiler_subclass',
        description   => 'use a HTML::Mason::Compiler subclass',
        interp_params => {
            compiler_class   => 'HTML::Mason::Compiler::ToObject::Test',
            use_object_files => 0,
            baz_val          => 75
        },
        component => <<'EOF',
