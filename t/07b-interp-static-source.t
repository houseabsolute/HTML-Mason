...
sub make_tests {
    ...
    foreach my $i ( 1 .. 4 ) {
        $group->add_support(
            path      => "support/remove_component$i",
    ...
    foreach my $i ( 1 .. 4 ) {
        $group->add_support(
            path      => "support/change_component$i",
    ...
    $group->add_test(
        name => 'change_component_without_static_source',
        description =>
          'test that on-the-fly component changes are detected with static_source=0',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'change_component_with_static_source',
        description =>
          'test that changing component has no effect with static_source=1',
...
EOF
    ...
    my $static_source_touch_file =
      File::Spec->catfile( $group->base_path, '.__static_source_touch' );
    $group->add_test(
        name => 'change_component_with_static_source_touch_file',
        description =>
          'test that changing component has no effect until touch file is touched',
        interp_params => {
            static_source            => 1,
...
EOF
    ...
    $group->add_test(
        name => 'remove_component_without_static_source',
        description =>
          'test that removing source causes component not found with static_source=0',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'remove_component_with_static_source',
        description =>
          'test that removing source has no effect with static_source=1',
        interp_params => { static_source => 1 },
        component     => <<'EOF',
<%init>
...
EOF
    ...
    $group->add_test(
        name => 'flush_code_cache_with_static_source',
        description =>
          'test that code cache flush & object file removal works with static_source=1',
        interp_params => { static_source => 1 },
        component     => <<'EOF',
