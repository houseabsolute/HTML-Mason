...
EOF
    ...
    $group->add_test(
        name         => 'allowed_globals2',
        description  => 'test that undeclared globals cause an error',
        pretest_code => sub {
            undef *HTML::Mason::Commands::global;
            undef *HTML::Mason::Commands::global;
        },    # repeated to squash a var used only once warning
        interp_params => { use_object_files => 0 },
        component     => <<'EOF',
<% $global = 1 %>
EOF
    ...
    $group->add_test(
        name        => 'allowed_globals3',
        description => 'test that declared globals are allowed',
        interp_params =>
          { use_object_files => 0, allow_globals => ['$global'] },
        component => <<'EOF',
<% $global = 1 %>
EOF
        expect => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'default_escape_flags_2_new',
        description =>
          'test that turning on default escaping works with new flags',
        interp_params => {
            use_object_files     => 0,
...
EOF
    ...
    $group->add_test(
        name => 'globals_in_default_package',
        description =>
          'tests that components are executed in HTML::Mason::Commands package by default',
        interp_params => {
            use_object_files => 0,
...
EOF
    ...
    $group->add_test(
        name          => 'globals_in_different_package',
...
EOF
    ...
    $group->add_test(
        name => 'line_nums',
        description =>
          'make sure that errors are reported with the correct line numbers',
        component => <<'EOF',
<% $x %> <% $y %>
<% $z %>
% die "Dead";
...
EOF
    ...
    $group->add_test(
        name => 'line_nums4',
        description =>
          'make sure that errors are reported with the correct line numbers in <%once> blocks',
        component => <<'EOF',
1
2
3
...
EOF
    ...
    $group->add_test(
        name => 'line_nums_with_escaped_newlines',
        description =>
...
EOF
    ...
    $group->add_test(
        name => 'line_nums_off_2',
        description =>
          'make sure that line number reporting is not off (another buggy case)',
        component => <<'EOF',
<%flags>
    inherit => undef
</%flags>
...
EOF
    ...
    $group->add_test(
        name => 'subcomp_parse_error',
        description =>
          'A misnamed block at the beginning of a component was throwing the lexer into an infinite loop.  Now it should be compiled into a component with a syntax error.',
        component => <<'EOF',
<%subcomp .foo>
 <% 5 %>
</%subcomp>
...
EOF
    ...
    $group->add_test(
        name => 'percent_at_end',
        description =>
          'Make sure that percent signs are only considered perl lines when at the beginning of the line',
        component => <<'EOF',
<% $x %>% $x = 5;
<% $x %>
<%init>
...
EOF
    ...
    $group->add_test(
        name => 'nameless_method',
        description =>
          'Check for appropriate error message when there is a method or def block without a name',
        component => <<'EOF',
<%method>
foo
</%method>
EOF
    ...
    $group->add_test(
        ...
        description =>
          'Check for appropriate error message when there is a method with an invalid name',
        component => <<'EOF',
<%method   >
foo
</%method>
...
EOF
    ...
    $group->add_test(
        name => 'no_strict_no_object_files',
        description =>
...
EOF
    ...
    $group->add_test(
        name => 'shared_to_init_global',
        description =>
          'Make sure <%init> can see global variables in <%shared>',
...
EOF
    ...
    $group->add_test(
        name => 'qw_in_perl_lines',
        description =>
          'Make sure that Mason that a qw() list stretching across multiple perl-lines works',
        component => <<'EOF',
% foreach my $foo ( qw( a
%                       b ) ) {
<% $foo %>
...
EOF
    ...
    $group->add_test(
        name => 'subcomp_leak',
        description =>
          'Make sure subcomps from one component do not show up in other components',
        component => <<'EOF',
<%init>
$m->scomp('has_subcomp');
$m->scomp('no_subcomp');
...
EOF
    ...
    $group->add_test(
        name => 'use_source_line_numbers_1',
        description =>
          'test presence of line directives when use_source_line_numbers is 1 (default)',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'use_source_line_numbers_0',
        description =>
          'test absence of line directives when use_source_line_numbers is 1',
...
EOF
    ...
    $group->add_test(
        name => 'compiler_id_change',
        description =>
          'Make sure different compiler params use different object dirs',
        component => <<'EOF',
<%args>
$count => 0
$compiler_params => {}
...
EOF
    ...
    $group->add_test(
        name => 'no_warnings',
        description =>
          'Make sure no warnings are generated for trying to output undef',
        component => <<'EOF',
% my $x;
x is <% $x %>
EOF
...
EOF
    ...
    $group->add_test(
        name => 'no_warnings_without_autoflush',
        description =>
          'Make sure no warnings are generated for trying to output undef when enable_autoflush is off',
...
EOF
    ...
    $group->add_test(
        name => 'warnings',
        description =>
          'Make sure that warnings _are_ generated for other bad use of uninit',
        component => <<'EOF',
% my $x;
x is <% $x + 2 %>
EOF
...
EOF
    ...
    $group->add_test(
        name => 'warnings_without_autoflush',
        description =>
          'Make sure that warnings _are_ generated for other bad use of uninit when enable_autoflush is off',
        interp_params => { enable_autoflush => 0 },
        component     => <<'EOF',
% my $x;
x is <% $x + 2 %>
...
EOF
    ...
    $group->add_test(
        name          => 'unbalanced_content_block_error',
...
EOF
    ...
    $group->add_test(
        name => 'unbalanced_content_block_subcomp_error',
        description =>
          'Detect and report unbalanced </&> tags in subcomponents',
        interp_params => { enable_autoflush => 0 },
        component     => <<'EOF',
<%def test>
</&>
