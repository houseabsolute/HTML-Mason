...
EOF
    ...
    $group->add_support(
        path      => '/dhandler_test/subdir/dhandler',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => '/dhandler_test/subdir/autohandler',
        component => <<'EOF',
Header
<% $m->call_next %>
EOF
    ...
    $group->add_support(
        path      => '/dhandler_test/bar/dhandler',
        component => <<'EOF',
dhandler = <% $m->current_comp->title %>
dhandler arg = <% $m->dhandler_arg %>
EOF
    ...
    $group->add_support(
        path      => '/dhandler_test/buff/dhandler',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'dhandler2',
        description =>
          'real comp to make sure the real comp is invoked, not the dhandler',
...
EOF
    ...
    $group->add_test(
        name => 'dhandler3',
        description =>
          'real comp declines the request to make sure the dhandler is invoked',
...
EOF
    ...
    $group->add_test(
        name => 'dhandler4',
        description =>
          'declines twice to make sure higher level dhandler is called',
...
EOF
    ...
    $group->add_test(
        name => 'dhandler6',
        description =>
          'test that a dhandler more than one directory up is found',
...
EOF
    ...
    $group->add_test(
        name        => 'contained_accessor_validate',
...
EOF
    ...
    $group->add_test(
        name => 'top_level_dhandler_handles',
        description =>
          'make sure dhandler at /dhandler is called correctly after decline from lower-level dhandler',
        path      => '/notused',
...
EOF
    ...
    $group->add_test(
        name        => 'top_level_dhandler_declines',
