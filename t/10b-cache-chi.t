...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_expires_in',
        component => <<'EOF',
x is <% $x %>
<%args>
$x
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_expire_in',
        component => <<'EOF',
x is <% $x %>
<%args>
$x
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_expire_if',
        component => <<'EOF',
x is <% $x %>
<%args>
$x
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_with_key',
        component => <<'EOF',
x is <% $x %>
<%args>
$x
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_and_die',
        component => <<'EOF',
<%init>
return if $m->cache_self;
die "argh!";
...
EOF
    ...
    $group->add_test(
        name => 'cache_self_error',
        description =>
          'test $m->cache_self with an error to make sure errors are propogated',
        %chi_interp_params,
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'cache_self_scomp',
        description =>
          'make sure that $m->cache_self cooperates with $m->scomp',
        %chi_interp_params,
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_filtered',
        component => <<'EOF',
x is <% $x %>
<%args>
$x
...
EOF
    ...
    $group->add_test(
        name => 'cache_self_filtered_scomp',
        description =>
          'test $m->cache_self with a filter block callled via $m->scomp',
        %chi_interp_params,
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_filtered_2',
        component => <<'EOF',
x is <% $x %>
<%args>
$x
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_abort2',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_with_subexec2',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => 'support/cache_self_with_subexec',
        component => <<'EOF',
% return if $m->cache_self;
...
EOF
    ...
    $group->add_test(
        name        => 'declined/cache_self_decline',
...
EOF
    ...
    $group->add_test(
        name          => 'data_cache_defaults',
        description   => 'modifying data_cache_defaults',
        interp_params => {
            data_cache_api      => 'chi',
            data_cache_defaults => { driver => 'Memory', global => 1 }
        },
        component => <<'EOF',
