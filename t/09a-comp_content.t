...
EOF
    ...
    $group->add_test(
        ...
        description =>
          'Tests a filter which outputs the content multiple times, with different values',
        component => <<'EOF',
% my $a;
<ul>
...
EOF
        expect_error =>
          'Component name in ending tag \(\.outer\) does not match component name in beginning tag \(\.inner\)',
    );

    #------------------------------------------------------------

    $group->add_test(
        name        => 'ending_tag_expr',
...
EOF
        expect_error =>
          'Cannot use an expression inside component with content ending tag',
    );

    #------------------------------------------------------------

    $group->add_test(
        name => 'multiline_open_close',
        description =>
          'Tests multiline opening and closing blocks for component with content call tags',
        component => <<'EOF',
<&|
Wrap
&>\
