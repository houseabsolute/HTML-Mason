...
sub make_tests {
    ...
    $group->add_support(
        path      => '/support/subrequest_error_test',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name        => 'subrequest_with_autohandler',
...
EOF
    ...
    $group->add_test(
        name      => 'subreq_exec_order',
        path      => '/subrequest2/subreq_exec_order',
        call_path => '/subrequest2/subreq_exec_order',
        description =>
          'Test that output from a subrequest comes out when we expect it to.',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => '/support/autoflush_subrequest',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'autoflush_subrequest',
        description =>
          'make sure that a subrequest respects its parent autoflush setting',
...
EOF
    ...
    $group->add_test(
        name => 'subrequest_inherits_no_autoflush',
        description =>
          'make sure that a subrequest inherits its parent autoflush setting (autoflush off)',
...
EOF
    ...
    $group->add_test(
        name => 'autoflush_in_subrequest',
        description =>
          'make sure that a subrequest with autoflush on does not flush parent',
        component => <<'EOF',
My child says:
% $m->flush_buffer;
...
EOF
    ...
    if (0) {
        $group->add_test(
            name => 'autoflush_in_parent_not_subrequest',
            description =>
              'make sure that a subrequest with autoflush can clear its own buffers',
            interp_params => { autoflush => 1 },
            component     => <<'EOF',
My child says:
% $m->flush_buffer;
...
EOF
    ...
    $group->add_test(
        name => 'return_scalar',
        description =>
          'tests that exec returns scalar return value of top component',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'return_list',
        description =>
          'tests that exec returns list return value of top component',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => 'support/subexec_recurse_test',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        ...
        expect_error =>
          qr{subrequest depth > 32 \(infinite subrequest loop\?\)},
    );
