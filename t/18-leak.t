...
sub make_tests {
    ...
    $group->add_test(
        name => 'interp_destroy',
        description =>
          'Test that interps with components in cache still get destroyed',
        component => <<'EOF',
<%perl>
{ 
...
EOF
    ...
    $group->add_support(
        path      => '/support/compile_error_comp',
        component => <<'EOF',
<%
EOF
    );
    ...
    $group->add_support(
        path      => '/support/runtime_error_comp',
        component => <<'EOF',
% die "bleah";
EOF
    );
    ...
    $group->add_support(
        path      => '/support/recursive_caller_1',
        component => <<'EOF',
<%perl>
...
EOF
    ...
    $group->add_support(
        path      => '/support/recursive_caller_2',
        component => <<'EOF',
<%perl>
...
EOF
    ...
    $group->add_test(
        ...
        description =>
          'Test that requests get destroyed after top-level component error',
...
EOF
    ...
    $group->add_test(
        name => 'component_destroy',
        description =>
          'Test that components get freed when cleared from the main cache',
...
EOF
    ...
    $group->add_test(
        name          => 'component_destroy_static_source',
...
EOF
    ...
    $group->add_test(
        name => 'subcomponent_destroy',
        description =>
          'Test that defs and methods don\'t cause components to leak',
        interp_params => {
            subcomp_class       => 'SubcomponentWatcher',
