...
sub make_tests {
    ...
    $outside_comp_root_test_file =
      dirname( $group->comp_root ) . "/.outside_comp";
...
EOF
    ...
    $group->add_test(
        name        => 'ampersand_with_funny_name',
...
EOF
    ...
    $group->add_test(
        ...
        description =>
          'test that various paths are canonicalized to the same component',
        component => <<'EOF',
<%perl>
...
EOF
    ...
    $group->add_test(
        name => 'fetch_comp_no_arg',
        description =>
          'fetch_comp with blank or undefined argument returns undef',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name        => 'outside_comp_root_prepare',
        description => 'test that file exists in dist/t for next two tests',
        pre_code    => sub {
            local *F;
            open( F, ">$outside_comp_root_test_file" );
            print F "hi";
        },
        component =>
          "test file '$outside_comp_root_test_file' <% -e '$outside_comp_root_test_file' ? 'exists' : 'does not exist' %>",
    ...
    $group->add_test(
        name => 'outside_comp_root_absolute',
        description =>
          'cannot call components outside comp root with absolute path',
        component => <<'EOF',
<& /../.outside_comp &>
EOF
        expect_error =>
          qr{could not find component for path '/../.outside_comp'},
    );

    #------------------------------------------------------------

    $group->add_test(
        name => 'outside_comp_root_relative',
        description =>
          'cannot call components outside comp root with relative path',
        component => <<'EOF',
<& ../../.outside_comp &>
EOF
        expect_error =>
    ...
    $group->add_support(
        path      => '/../outside_comp_root_from_top',
        component => <<'EOF',
<& ../.outside_comp &>
EOF
    );
    ...
    $group->add_test(
        name => 'outside_comp_root_relative_from_top',
        description =>
          'cannot call components outside comp root with relative path from component at top of root',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name        => 'parent_designator_with_no_parent',
...
EOF
        expect_error =>
          qr/PARENT designator used from component with no parent/,
    );
