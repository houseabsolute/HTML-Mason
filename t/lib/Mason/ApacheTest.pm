...
use base 'Exporter';
our @EXPORT_OK =
  qw( require_libapreq require_cgi require_apache_filter chmod_data_dir );
...
INIT {
    ...
    {
        plan skip_all => '$TestConfig->{is_maintainer} is not true or '
...
EOF
    ...
    _write_comp(
        'multiconf1/autohandler', <<'EOF'
<& $m->fetch_next, autohandler => 'present' &>
EOF
    );

    _write_comp(
        'multiconf1/autohandler_test', <<'EOF'
<%args>
...
EOF
    ...
    _write_comp(
        'with_dhandler_no_ct/dhandler', <<'EOF',
...
sub _start_httpd {
    ...
    my $conf_file =
      File::Spec->catfile( $TestConfig->{apache_dir}, 'conf', 'httpd.conf' );
    my $pid_file =
      File::Spec->catfile( $TestConfig->{apache_dir}, 'logs', 'httpd.pid' );
...
sub _cleanup_data_dir {
    ...
    foreach ( grep { -d File::Spec->catdir( $dir, $_ ) && $_ !~ /^\./ }
        readdir $dh )
    {
...
EOF
    ...
    my $expected_class =
      $p{with_handler} ? 'My::Interp' : 'HTML::Mason::Interp';

    push @tests, {
        path   => '/comps/interp_class',
        expect => <<"EOF",
X-Mason-Test: Initial value
...
sub _apache_request_with_handler_tests {
    ...
    return (
        {
            path   => '/ah=4/comps/apache_request',
            expect => <<'EOF',
X-Mason-Test: Initial value
Status code: 0
EOF
...
EOF
        ...
        },
        {
            path => '/perl-status',
            regex =>
              qr{<a href="/perl-status\?mason0001">HTML::Mason status</a>},
        },
    );
}
...
sub _filter_response {
    ...
    my $actual;
    {
        ...
        if ( $p->{with_handler} ) {
            ...
            $val = (
                defined $response->headers->header('X-Mason-Test')
    ...
    $response->headers->scan(
        sub {
            return if $_[0] eq 'X-Mason-Test' || $_[0] !~ /^X-Mason/;
    ...
    if ( ( $test->{method} && $test->{method} eq 'HEAD' )
        || !$p->{with_handler} )
    {
...
sub _check_output {
    ...
    if ( $test->{expect} ) {
        ...
        like( $output, $test->{regex}, "Regex test for $desc" );
    }
    elsif ( !$test->{extra} ) {
