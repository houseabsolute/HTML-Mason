...
if ($alarm_works) {
    ...
    my $error = (
          $alarm          ? "entered endless while loop"
...
my ($comp_root) =
  File::Spec->catdir( getcwd(), 'mason_tests', 'comps' ) =~ /(.*)/;
my ($data_dir) =
  File::Spec->catdir( getcwd(), 'mason_tests', 'data' ) =~ /(.*)/;
...
my $comp2 = HTML::Mason::ComponentSource->new(
    ...
    source_callback => sub {
        read_file(
            File::Spec->catfile( File::Spec->curdir, 't', 'taint.comp' ) );
    },
);
ok $comp2;
...
eval {
    $interp->compiler->compile_to_file(
