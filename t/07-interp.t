...
sub make_tests {
    ...
    $group->add_support(
        path      => '/autohandler_test/autohandler',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name        => 'no recursive autohandlers',
...
EOF
    ...
    $group->add_support(
        path      => '/autohandler_test/subdir/plainfile',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'alternate autohandler name',
        description =>
          'tests that providing an alternate name for autohandlers works',
        call_path     => '/autohandler_test/subdir/hello',
...
EOF
    ...
    my $alt_root =
      File::Spec->catdir( HTML::Mason::Tests->tests_class->base_path,
        'alt_root' );
    my @roots = (
        [ main => HTML::Mason::Tests->tests_class->comp_root ],
        [ alt  => $alt_root ]
    );

    #HACK!
    HTML::Mason::Tests->tests_class->write_comp(
        '/alt_root/interp/comp_root_test/private2',
...
EOF
    ...
    HTML::Mason::Tests->tests_class->write_comp(
        '/alt_root/interp/comp_root_test/shared',
        File::Spec->catdir( $alt_root, 'interp', 'comp_root_test' ),
        'shared', <<'EOF' );
...
EOF
    ...
    $group->add_test(
        name => 'shared',
        description =>
          'test that component in both comp_roots is called in first comp_root',
...
EOF
    ...
    $group->add_test(
        ...
        expect_error =>
          qr{32 levels deep in component stack \(infinite recursive call\?\)},
    );
...
EOF
    ...
    $group->add_support(
        path      => '/support/code_cache/show_code_cache',
        component => <<'EOF',
...
EOF
    ...
    foreach my $i ( 1 .. 7 ) {
        $group->add_support(
            path      => "/support/code_cache/plain$i",
            component => "",
        );
    }

    $group->add_support(
        path      => "/support/code_cache/call_plain_comps",
        component => <<'EOF',
...
EOF
    ...
    $create_code_cache_test->(
        'unlimited', 'plain1, plain2, plain3, plain4, plain5, plain6, plain7'
    );
    $create_code_cache_test->( 0, '' );
    $create_code_cache_test->( 4, 'plain1, plain2, plain7' );
    $create_code_cache_test->(
        8, 'plain1, plain2, plain3, plain5, plain6, plain7'
    );
    ...
    $group->add_test(
        name => 'dhandler_name',
        description =>
          'Test that providing an alternate name for dhandlers works',
...
EOF
    ...
    $group->add_support(
        path      => 'preloads_test/show_code_cache',
        component => <<'EOF',
Code cache contains:
...
EOF
    ...
    $group->add_support(
        path      => 'preloads_test/subdir/in_a_subdir',
        component => 'howareyou',
    );
...
EOF
    ...
    $group->add_test(
        name => 'preload_3',
        description =>
          'Preload all components (including subdirectory) by glob pattern',
        interp_params => {
            preloads =>
              [ '/interp/preloads_test/*', '/interp/preloads_test/*/*' ]
        },
        component => <<'EOF',
<& preloads_test/show_code_cache &>
EOF
...
EOF
    ...
    $group->add_test(
        name => 'process_comp_path',
        description =>
          'Test that component paths cannot be resolved outside the comp root',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'process_comp_path2',
        description =>
          'Test that component paths containing /../ work as long they stay in the comp root',
...
EOF
    ...
    $group->add_test(
        ...
        description =>
          'test that warnings during component compilation cause an exception except for redefined subs',
        component => <<'EOF',
a global: <% $GLOBAL %>
<%once>
sub foo { 1 }
...
EOF
        expect_error =>
          qr/Global symbol "\$GLOBAL" requires explicit package name/,
    );
    ...
    $group->add_test(
        ...
        interp_params =>
          { ignore_warnings_expr => qr/useless use of "re" pragma/i },
        component => <<'EOF',
% use re;
foo
EOF
...
EOF
    ...
    $group->add_test(
        name          => 'ignore_all_warnings',
        description   => 'test ignoring all warnings',
        interp_params => { ignore_warnings_expr => '.' },
        component     => <<'EOF',
<%once>
sub foo { 1 }
sub foo { 1 }
...
EOF
    ...
    if ( load_pkg('Cache::Cache') && load_pkg('Cache::MemoryCache') ) {
        $group->add_test(
            ...
            interp      => HTML::Mason::Tests->tests_class->_make_interp(
                comp_root => HTML::Mason::Tests->tests_class->comp_root
            ),
            component => <<'EOF',
Hello World!
<% ref $m->cache %>
...
EOF
    ...
    $group->add_test(
        name => 'make_component_error',
        description =>
          'make sure a proper exception is thrown with make_component syntax errors',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => '/support/corrupt_object_file',
        component => "I was loaded\n",
    );

    $group->add_test(
        name => 'corrupt_object_file',
        description =>
          'test that Mason can recover from a corrupt or empty object file',
        component => <<'EOF',
<%init>
