...
EOF
    ...
    $group->add_test(
        name => 'req_obj',
        description =>
          'tests various operations such as comp calls, $m->current_comp',
        component => <<'EOF',
<%def .subcomp>
% if ($count < 5) {
...
EOF
    ...
    $group->add_test(
        ...
        call_args => { junk => 5 },
        description =>
          'tests caller, callers, fetch_comp, process_comp_path, comp_exists and scomp',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => '/autohandler_test2/autohandler',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => '/autohandler_test2/dir1/autohandler',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'autoflush_printf',
        description =>
          'Test printf function from a component with autoflush on',
        interp_params => { autoflush => 1 },
        component     => <<'EOF',
This is first.
% printf '%s', "This is second.\n";
...
EOF
    ...
    $group->add_test(
        name => 'flush_print',
        description =>
          'Test print function from a component in conjunction with $m->flush_buffer call',
        component => <<'EOF',
This is first.
% print "This is second.\n";
% $m->flush_buffer;
...
EOF
    ...
    $group->add_test(
        name => 'flush_print_autoflush',
        description =>
          'Test print function from a component with autoflush on in conjunction with $m->flush_buffer call',
        interp_params => { autoflush => 1 },
        component     => <<'EOF',
This is first.
% print "This is second.\n";
...
EOF
    ...
    $group->add_test(
        name => 'abort_and_filter',
        description =>
          'Test that an abort in a filtered component still generates _some_ output, and that filter is run only once',
        component => <<'EOF',
filter
...
EOF
    ...
    $group->add_test(
        name => 'abort_and_filter_2',
        description =>
          'Test that $m->aborted can be checked in a filter section',
        component => <<'EOF',
filter
...
EOF
    ...
    $group->add_test(
        name => 'abort_and_store',
        description =>
          'Test that an abort in a store\'d component still generates _some_ output',
        component => <<'EOF',
filter
...
EOF
    ...
    $group->add_support(
        path      => '/support/subdir/longjump_test2',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'longjump',
        description =>
          'Accidentally calling next to exit a component does not corrupt stack',
        component => <<'EOF',
...
EOF
    ...
    $group->add_support(
        path      => '/support/callers_out_of_bounds2',
        component => <<'EOF',
hi
EOF
    );
    ...
    $group->add_support(
        path      => '/support/callers_out_of_bounds1',
        component => <<'EOF',
...
EOF
    ...
    $group->add_test(
        name => 'call_self_retval',
        description =>
          'Test that we can get return value of component via $m->call_self',
        component => <<'EOF',
called
<%init>
my @return;
...
EOF
    ...
    $group->add_test(
        name => 'call_self_output_and_retval',
        description =>
          'Test that we can get return value and output of component via $m->call_self',
        component => <<'EOF',
called
<%init>
my $out;
...
k2: <% $m->notes->{key} %>
EOF
        expect => qr/k: value\s+k2: value/,
    );

    #------------------------------------------------------------

    $group->add_test(
        name => 'flush_and_store',
        description =>
          q{Test that $m->flush_buffer is ignored in a store'd component},
        interp_params => { autoflush => 1 },
        component     => <<'EOF',
<%def .world>\
World\
...
EOF
    ...
    $group->add_test(
        name => 'flush_and_scomp_recursive',
        description =>
          'Test that $m->flush_buffer is ignored in a recursive scomp() call',
        interp_params => { autoflush => 1 },
        component     => <<'EOF',
<%def .orld>\
orld\
