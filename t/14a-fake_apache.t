...
%ENV = (
    'SCRIPT_NAME'          => '/login/welcome.html',
    'REQUEST_METHOD'       => 'GET',
...
# Add some cookies
ok(
    $r->headers_out()->add( 'Set-Cookie' => 'AF_SID=6e8834d8787ee57a; path=/' ),
    "Set cookie"
);
ok(
    $r->headers_out()->add(
        'Set-Cookie' =>
          'uniq_id=5608074; path=/; expires=Tue, 26-Aug-2008 21:27:03 GMT'
    ),
    "Set cookie"
);
...
is( $r->subprocess_env('CONTENT_LENGTH'), 42,
    "Check CONTENT_LENGTH env again" );
is( $r->subprocess_env('content_length'), 42,
    "Check content_length env again" );
...
like(
    $headers,
    qr|Content-Type: text/xml(?:; charset=ISO-8859-1)?|i,
    "Check content type"
);
like(
    $headers,
    qr|Set-Cookie: AF_SID=6e8834d8787ee57a; path=/|i,
    'Check first cookie'
);
like(
    $headers,
    qr|Set-Cookie: uniq_id=5608074; path=/; expires=Tue, 26-Aug-2008 21:27:03 GMT|i,
    'Check second cookie'
);
...
SKIP:
{
    ...
    stdout_is( sub { $r->print('Foo bar') },
        'Foo bar', 'print does not include the object itself' );
}
