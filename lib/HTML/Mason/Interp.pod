...
=item make_anonymous_component (comp=>...)

=item make_anonymous_component (comp_file=>...)

This method compiles Mason component source code and returns a
Component object.  The source may be passed in as a string in C<comp>,
...
If Mason encounters an error during processing, an error will be thrown.

The returned value is the only reference to the new component object,
i.e. Mason will not make the new component available to other components.
For that behavior, see C<make_component()>.

Example of usage:

    # Make a component
    my $comp =
      eval { $interp->make_anonymous_component
...
=item make_component (comp=>..., path=>...)

=item make_component (comp_file=>..., path=>...)

This method compiles Mason component source code just like
C<make_anonymous_component()>, and then installs the component into
the interpreter so that it can be called from other components.  The
Mason calling path is specified via the C<path> parameter.
