...
own directories. See the L<Devel/autohandlers> section of the
...
Overrides the time and date returned by $m->time and $m->date with a
...
With no current_time parameter (the default), $m->time and $m->date
...
Apache or the O/S.) The default mode is batch.  See L<Admin/out_mode>
...
Absolute path to prepend to relative filenames passed to C<$m->file()>. Does
not require a trailing slash. For example, if the file root is
'/foo/bar', then C<$m->file('baz/bap')> will read the file
'/foo/bar/baz/bap'. Undefined by default; if left undefined,
relative path names to C<$m->file()> are prepended with the
...
as they are with C<$m->comp>. exec returns the return value of
