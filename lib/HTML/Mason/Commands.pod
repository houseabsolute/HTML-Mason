...
Returns the absolute path of the component interrupted by an
autohandler.  See the L<Components/autohandlers> section of the I<Component
Developer's Guide> for usage and examples.
...
Uses mc_cache to cache the entire output of the current component.  It is typically
used right at the top of an C<E<lt>%init%E<gt>> section:
...
The "return if" is necessary. mc_cache_self handles both the retrieve
and store, so you can pass both kinds of options to it. See
mc_cache for an explanation of options.

mc_cache_self uses a bit of magic to accomplish everything in one
line.  You don't really need to understand this, but if you're
...
o When control is returned to mc_cache_self, it stores the content in
the cache and also outputs the content normally.  Finally
mc_cache_self returns 1, which in turn causes foo to return
immediately.
