...
To cache the component's return value:
    ...
    <%init>
        my ($retval,$cached) = mc_cache_self(expire_in=>'3 hours'[, key=>'fookey']);
...
o foo again calls mc_cache_self, which immediately returns 0 this time.
...
Finally mc_cache_self returns the list (retval,1) which in turn
