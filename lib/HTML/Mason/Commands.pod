...
B<HTML::Mason::Devel> (the I<Mason Developer's Guide>), for usage
examples.
...
C<$m->auto_comp> from a component other than the autohandler and a
forward relative path cannot be generated. See the L<Devel/autohandlers>
section of the I<Component Developer's Guide> for usage and examples.

=item $m->cache ([action=>'retrieve|store|expire|keys'], [key=>name],
...
=item $m->cache_self (... same as $m->cache options...)
...
=item $m->call_self ([output_ref], [retval_ref])
...
<%init> section:
    ...
    <%init>
        if $m->call_self(my \$output, my \$retval) {
            ... < perform filter and return > ...
        }
        ... <rest of init> ...
    </%init>

See the L<Devel/filtering> section of the I<Component Developer's Guide>
for usage and examples.

=item $m->comp =for html <a name="comp">
...
component:
    ...
    my $lastmod = (stat($m->comp_source))[9];
...
use $m->date() rather than UnixDate directly.

$m->date() works faster than UnixDate for some formats by caching results. 
...
L<Devel/dhandlers> section of the I<Component Developer's Guide>
for examples.

=item $m->file (filename)
...
$m->out() should be favored over C<print> or C<$r-E<gt>print>,
...
To allow time and date simulations in the previewer, components should
use $m->time() rather than calling time() directly.
...
=item $m->suppress_http_header (0|1)
...
$m->suppress_http_header(1) somewhere in the C<E<lt>%init%E<gt>>
block. You can call $m->suppress_http_header(0) to cancel.
