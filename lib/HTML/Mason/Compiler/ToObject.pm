...
sub compile_to_file {
    ...
    $self->compile(
        comp_source => $source->comp_source_ref,
...
sub compiled_component {
    ...
    $self->_output_chunk( $p{fh}, \$obj_text,
        $self->_constructor( $self->comp_class, $params ), ';', );
...
sub _compile_subcomponents_or_methods {
    ...
    foreach ( keys %{ $self->{current_compile}{$type} } ) {
        ...
        local $self->{current_compile}->{in_named_block} =
          { type => $type, name => $_ };
...
sub _component_params {
    ...
    if ( $self->named_component_subs ) {
        $params{code} = '\\&'
          . $self->_sub_name( grep { defined }
              @{ $self->{current_compile}{in_named_block} }{ 'type', 'name' } );
...
sub _set_buffer {
    ...
    if ( $self->enable_autoflush ) {
        ...
        return
          'my $_outbuf = $m->{top_stack}->[HTML::Mason::Request::STACK_BUFFER];'
          . "\n";
    }
}

my %coercion_funcs = (
    '@' => 'HTML::Mason::Tools::coerce_to_array',
...
EOF
    ...
    my @required = (
        map  { $_->{name} }
        grep { !defined $_->{default} } @{ $self->{current_compile}{args} }
    );
...
EOF
    ...
    foreach ( @{ $self->{current_compile}{args} } ) {
        ...
        if ( $coercion_funcs{ $_->{type} } ) {
            $coerce =
              $coercion_funcs{ $_->{type} } . "( $arg_in_array, '$var_name')";
        }
        else {
            $coerce = $arg_in_array;
        }
