...
The E<lt>%initE<gt>, E<lt>%cleanupE<gt>, and E<lt>%onceE<gt> sections
contain blocks of Perl that execute at specific times.
...
Each instance of this section creates a <i>subcomponent</i> embedded inside
...
etc. (The only prohibited tags are C<E<lt>%defE<gt>> and
C<E<lt>%methodE<gt>>, but these will be supported eventually).
...
must be valid Perl hash keys and values respectively.
...
your administrator hasn't touched out_mode then it is probably
'batch', the default.
...
However, sometimes you'll need a more flexible template: one that
adjusts somewhat to the requested page.  You might want to allow each
page or subsection to specify a title, background color, or logo image
while leaving the rest of the template intact. You might want some
pages or subsections to use a different template, or to ignore
templates entirely.

In this section you will learn how to address these issues with the
object-oriented style primitives introduced in Mason 0.85.
...
architecture if the current experiment proves successful.)
...
=head2 Creating flexible templates
...
Each of these templates can access methods and/or attributes of the
requested page. First, use C<$m-E<gt>called_comp> to get a handle on
...
or alternatively a path of the form 'comp:method':

    <& SELF:header &>
    $m->comp('SELF:header')

where special component path SELF is shorthand for C<$m-E<gt>called_comp>.
...
'method_exists' to avoid this error for questionable attributes:

    if ($self->attr_exists('color')) { ...

To defer to a method or attribute of your parent, use C<parent>:

    $m->current_comp->parent->call_method('header')
    $m->current_comp->parent->attr('color')

or alternatively, for methods, the special component path PARENT:
...
/products/autohandler does the same sort of thing, overriding some attributes 
and methods for the /products section of the site. 
...
/products/index.html maybe overrides a few attributes, but mainly provides
