...
B<E<lt>%perlE<gt> xxx E<lt>%/perlE<gt>>
...
The filtering component can be called in all the same ways a normal component
is called, with arguments and so forth.  The only difference between a 
filtering component and a normal component, is that a filtering component 
is expected to fetch the content by calling $m->content, and do something
with it.
...
correct one based on a global $lang variable, which would be setup
...
$m->content will return undef.  If a normal component which does not call  
$m->content is called with content, the content will not be output.

If you wrap a filtering component call around the entire component, 
the result will be functionally similar to a <%filter> section.
...
The tricky part of using filter components as control
structures is setting up variables which can be accessed from
both the filter component and the content, which is in the component 
which calls the filter component.  The content has access to all 
variables in the surrounding component, but the filtering component
does not.  There are two ways to do this: 
use global variables, or pass a reference to a lexical variable to the 
filter component.
...
must make sure that "$var" is included in Mason's allow_globals 
parameter.  Local-izing $var within the filter component will allow 
the loop components to be nested.
...
for this component. Here's the typical usage of C<$m-E<gt>cache>:
    ...
    my $result = $m->cache->get('identifier');
    if (!defined($result)) {
        ... compute $result ...
        $m->cache->set('identifier', $result);
    }
...
=head2 Multiple Identifiers/Values

A cache can store multiple identifier/value pairs. A value can be
...
You can fetch all the identifiers in a cache with

    my @idents = $m->cache->get_identifiers;

It should be noted that Mason reserves all identifiers beginning with
...
It is typically used right at the top of a <%init%> section:
...
C<$m->E<gt>cache-E<gt>get_object> returns the B<Cache::Object> associated
with a particular identifier. You can use this to retrieve useful
meta-data:
...
By default C<$m-E<gt>cache> uses B<Cache::FileCache>, but you can override
...
"Cache::" is automatically prepended. For example:
    ...
    my $result = $m->cache(cache_class=>'MemoryCache')->get('identifier');
    $m->cache(cache_class=>'MemoryCache')->set('identifier', $result);

You can even specify different subclasses for different identifiers in the
...
have used for a given component or identifier.
