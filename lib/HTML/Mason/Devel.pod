...
Here's how a simple /newsfeeds/dhandler might look:
	...
	(qq{SELECT headline,body FROM news
            WHERE section='$section' AND story='$story'});
...
return() from a C<E<lt>%onceE<gt>> section; the current parser
...
Do not attempt to return() from a C<E<lt>%sharedE<gt>> section; the current
parser cannot properly handle it.
