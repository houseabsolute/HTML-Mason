...
capabilities and implementation, then B<HTML::Mason::Admin> is for you
too.
...
still be useful, of course.  Also make sure to read L<NON-WEB MASON in
Admin|HTML::Mason::Admin/NON-WEB MASON> in the Administrator's Guide.
...
execute in the order they appear. Other sections (C<< <%init> >>, C<<
<%args> >>, etc.) are tied to component events like initialization,
...
Five system objects share the work of serving requests in Mason:
...
If a filter component is called like a normal component (e.g. <& itext
&>), C<< $m->content >> will return undef.  If a normal component
which does not call C<< $m->content >> is called with content, the
...
Request object's constructor, such as C<autoflush> or C<out_method>.
Once you have a new request object you simply call its C<exec> method
to execute it, which takes exactly the same parameters as the C<comp>
method.
...
simply pass an C<out_method> parameter to C<< $m->make_subrequest >>:
...
can let in directory requests as well; see L<Allowing directgory
request in Admin|HTML::Mason::Admin/"Allowing directory requests">.
...
the L<Interp's dhandler_name|HTML::Mason::Interp/dhandler_name>
parameter.
...
with the L<Interp's
autohandler_name|HTML::Mason::Interp/autohandler_name> parameter.
...
As with C<< <%once> >>, variables declared in this section can be seen in all
...
one more subcomponents or methods. See L<Object-Oriented
Techniques|"Object-Oriented Techniques"> for an example of
usage.
...
section.  However, you can access arguments via the C<<
$m->request_args >> method, documented in the L<Request
docs|HTML::Mason::Request/"request_args">.
...
most once per request. C<< <%init> >> offers more a predictable execution
time.

Any component with a C<< <%shared> >> section incurs an extra performance
...
C<< <%shared> >> when patently unnecessary, e.g. when an C<< <%init> >> would work
as well.
...
components. For example, you could start subcomponent names with ".".
...
L<Compiler's
default_escape_flags|HTML::Mason::Compiler/default_escape_flags>
parameter. For example, if the administrator specifies
...
The C<< $m->cache >> method returns an object representing the cache
for this component. Here's the typical usage of C<< $m->cache >>:
...
I<__mason> for its own use.

=head2 Expiration

You may pass an optional third argument to C<< $m->cache->set >>
...
The C<expire_if> option from Mason 1.0x and earlier is currently not
supported, though a version of it may be added to C<Cache::Cache> in
the future. For now, you can use tests on C<get_object()> to
conditionally expire a cache. For example, this code expires C<$key>
whenever a particular C<$file> changes:

    if (my $co = $m->cache->get_object($key) and
        $co->get_created_at < (stat($file))[9]) {
        $m->cache->remove($key);
    }
...
One solution is to have the first process set the expiration time of
the value ahead a few minutes, so that the other processes use the old
value instead of also recomputing it:

   my $result = $m->cache->get('key');
   if (!defined($result)) {
       if (my $o = $m->cache->get_object('key')) {
           $o->set_expiration_time(time + 2*60);
       }
       ... compute $result ...
       $m->cache->set('key', $result);
   }

Of course, this solution will only work if it is acceptable for a
component to get a slightly stale cache value for a few minutes.

Mason 1.0x and earlier had a feature called "busy locks" that
addressed the same problem in an admittedly more compact fashion.
This feature is not currently supported, although a version of it may
be added to C<Cache::Cache> in the future.
...
C<< $m->cache_self >> is built on top of C<< $m->cache >>, so it
accepts all of the cache options described earlier.  C<<
$m->cache_self >> can also cache a component's return value; see the
B<L<HTML::Mason::Request|HTML::Mason::Request>> documentation for details.

=head2 Cache Object Meta-data

C<< $m->cache->get_object >> returns the B<Cache::Object> associated
...
subclasses. For example, B<Cache::FileCache> implements the interface with a
set of directories and files, while B<Cache::MemoryCache> implements the
interface in memory.

By default C<< $m->cache >> uses B<Cache::FileCache>, but you can override
...
C<HTML::Mason::Utils::data_cache_namespace>, given a component path,
...
know whether your server's default is to have C<autoflush> on or off.
...
L<Request's auto_send_headers|HTML::Mason::Request/auto_send_headers>
parameter available when running Mason with the ApacheHandler module.
...
C<< $m->abort >> bypasses the component stack and ensure that
...
Because it is so commonly needed, Mason provides an external redirect
method:
...
A method is called via the use of C<SELF:> or C<PARENT:>.  These are
