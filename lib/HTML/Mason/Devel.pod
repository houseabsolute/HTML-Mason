...
As an example, you might have a component C<is_netscape> that analyzes
...
While it is possible for a component to generate output B<and> return
values, there is very little reason for a component to do both. For
example, it would not be very friendly for C<is_netscape> to output
"hi Mom" while it was computing its value, thereby surprising the
C<if> statement! Conversely, any value returned by an output
...
L<Cache::Cache|Cache::Cache> or its newer sibling L<CHI|CHI>. You control which one Mason
uses with the P<data_cache_api> parameter. Currently C<Cache::Cache> is the default, but
C<CHI> will take over when it gets a little more stable.  The APIs are very
similar for Mason users, so that most of the information below applies to both; any
differences are noted.

=head2 Basic Usage

The C<< $m->cache >> method returns a cache object representing the cache for
...
Another solution, available only if you are using C<CHI>, is L<expires_variance>
which will create a time window during which expiration may occur. See the C<CHI>
...
C<< $m->cache->get_object >> returns a L<Cache::Object|Cache::Object> or
L<CHI::CacheObject|CHI::CacheObject> associated with a particular key. You can use this to
retrieve useful meta-data:
...
The L<Cache::Cache> API is implemented by a variety of backend subclasses. For
...
The L<CHI> API is implemented by a variety of drivers, for example
L<CHI::Driver::File|CHI::Driver::File>, L<CHI::Driver::FastMmap|CHI::Driver::FastMmap>,
and L<CHI::Driver::Memcached|CHI::Driver::Memcached>.

C<CHI::Driver::File> is the default, but you can override
this with the I<driver> keyword. The value must be the name of a
C<CHI::Driver> subclass; the prefix "CHI::Driver::" need not be included.
For example:
...
the namespace associated with the component. For C<Cache::Cache>, the function
C<HTML::Mason::Utils::data_cache_namespace>,
given a component id (usually just the component path), returns the
namespace. For C<CHI>, the component id/path itself is the namespace.
...
and you are using the default file backend:
