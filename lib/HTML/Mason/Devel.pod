...
APIs: L<Cache::Cache|Cache::Cache> or its newer sibling
L<CHI|CHI>. You control which one Mason uses with the
P<data_cache_api> parameter. Currently C<Cache::Cache> is the default,
but we will switch to C<CHI> once it is more mature.  The APIs are
very similar for Mason users, so that most of the information below
applies to both; any differences are noted.
...
L<expires_variance> which will create a variable time window during
...
L<CHI::Driver::File|CHI::Driver::File>,
L<CHI::Driver::FastMmap|CHI::Driver::FastMmap>, and
