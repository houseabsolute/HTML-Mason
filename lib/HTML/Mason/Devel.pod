...
Whenever a component that has a filter, or the components that it
calls, are executing, calls to C<< $m->flush_buffer() >> are ignored.
This is to ensure that the filter sees the entire component output as
a whole, all at once.  However, calls to C<< $m->clear_buffer() >> are
not ignored.
