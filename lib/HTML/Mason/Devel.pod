...
defer control to the next dhandler by calling $m->decline.
...
its actions, the autohandler typically calls C<$m->call_next> to
...
Most of the time autohandler can simply call C<$m->call_next> without
needing to know what component was called. However, should you need
it, the component object is available from C<$m->fetch_next>.  For
...
C<$m->fetch_next> is also useful for calling the component manually,
