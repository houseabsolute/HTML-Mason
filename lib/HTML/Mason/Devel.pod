...
Mason uses C<Log::Any> to log various events, such as the start and
end of each request and each component call. You can also log to
C<Log::Any> from a component with the
...
See C<Log::Any::Adapter> for how to direct these logs to
