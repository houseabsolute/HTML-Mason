...
The top-most autohandler runs first; each C<< $m->call_next >> transfers control to the
next autohandler and finally to the originally called component. This allows you, for
example, to combine general site-wide templates and more specific section-based templates.
...
A C<< <%cleanup> >> block is equivalent to a C<< <%perl> >> block at
...
APIs: C<Cache::Cache> or its newer sibling C<CHI>. You control which
one Mason uses with the P<data_cache_api> parameter. Currently
C<Cache::Cache> is the default, but we will switch to C<CHI> once it
...
The C<CHI> API is implemented by a variety of drivers, for example
C<CHI::Driver::File>, C<CHI::Driver::FastMmap>, and
C<CHI::Driver::Memcached>.

C<CHI::Driver::File> is the default, but you can override this with
the I<driver> keyword. The value must be the name of a C<CHI::Driver>
...
end of each request and each component call. You can also log to
L<Log::Any|Log::Any> from a component with the
L<log|HTML::Mason::Request/log> method. e.g.

    $m->log->error("Something bad happened!");
    $m->log->debugf("Arguments for '%s' were '%s'", $func, \%args)
        if $m->log->is_debug;

See L<Log::Any::Adapter|Log::Any::Adapter> for how to direct these logs to
