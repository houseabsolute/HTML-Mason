...
still be useful, of course.  Also make sure to read L<Admin/NON-WEB
MASON> in the Administrator's Guide.
...
o L<Data Caching|Data Caching>

o L<Sending HTTP Headers|Sending HTTP Headers>
...
=head2 Components that Filter Content
...
=head2 Advanced Components that Filter Content
...
For convenience, Mason also provides an C<< $m->subexec >> method that
internally calls C<< $m->make_subrequest >> and then calls C<exec> on
...
C<exec>.  If you wish to make multiple subrequests, we recommend
creating a new subrequest object for each one.  The effect of calling
C<exec> more than once on a given object is not predictable.
...
can let in directory requests as well; see L<Admin/Allowing directory
requests>.
...
The administrator can customize the file name used for dhandlers
with the L<Interp/dhandler_name|Interp/dhandler_name> Interp parameter.
...
separate section, L<Devel/Object-Oriented Techniques|Devel/Object-Oriented Techniques>.

The administrator can customize the file name used for autohandlers
with the L<Interp/autohandler_name|Interp/autohandler_name> Interp parameter.
...
templates more flexible. See L<Devel/Object-Oriented Techniques|Devel/Object-Oriented Techniques> for
more information.
...
Currently there is only one flag, C<inherit>, which specifies
the component's I<parent> in the form of a relative or absolute
component path. A component inherits methods and attributes from its
parent; see L<Devel/Object-Oriented Techniques|Devel/Object-Oriented Techniques> for examples.
...
You can use L<Components that filter Content|Components that filter Content> if you want to filter
specific parts of a component rather than the entire component.
...
L<Compiler/default_escape_flags|Compiler/default_escape_flags> Compiler parameter. For example, if
...
I<__cache_self__> for its own use.
...
XXX: Not if there are multiple comp root!
...
L<Request/auto_send_headers|Request/auto_send_headers> Request parameter available when running
Mason with the ApacheHandler module.
...
page. First, use C<< $m->base_comp >> to get a handle on the
