...
This manual has grown rather too large for its own good. It needs to
be split into at least two documents (tutorial and reference manual).
I've tried to keep related sections together, but this creates a
non-optimal order for beginners.

If you are just learning Mason and want to get started quickly, I
...
CALLING COMPONENTS
PASSING PARAMETERS
INITIALIZATION/CLEANUP (<%init> and <%cleanup> only)
DATA CACHING
...
Note to pre-0.8 users: The $m API replaces both the old C<mc_> command
set and the $REQ variable. See B<L<HTML::Mason::Commands>> for a
conversion guide.  There is also a utility provided with the
distribution, F<bin/convert0.8.pl>, that performs appropriate conversions
on your existing component tree.

=head2 System Objects

Three system objects share the work of serving requests in Mason:
Parser, Interp, and ApacheHandler. The administrator creates these
objects and provides parameters that shape Mason's behavior. As a pure
component developer you shouldn't need to worry about or access these
objects, but occasionally we'll mention a relevant parameter.
...
C<$m-E<gt>dhandler_arg> returns the same path_info stripped of the
...
directory requests as well; see L<Admin/Allowing requests for
directories>.
...
version 0.82, only the most specific autohandler would execute.  In
0.82 and beyond each autohandler gets a chance to run.  The top-most
autohandler runs first; each $m->call_next transfers control to the
...
This code executes once when the component is loaded. Its scope
includes all of a component's code, and its variables persist for the
...
Like <%once>, the scope of this section includes all of a component's
code: the main component, subcomponents, methods, and dynamic
attributes. However, the code runs once per request (whenever the
component is used) and its variables last only til the end of the
request.

Useful for initializing variables needed in, say, the main body and
one more methods. See L<Devel/Object-Oriented Techniques> for an
example of usage.

Avoid using <%shared> for side-effect code that needs to run at a
predictable time during page generation. Assume only that <%shared>
runs just before the first code that needs it and at most once
per request. <%init> offers more a predictable execution time.
...
<%shared> when patently unnecessary, e.g. when an <%init> would work
as well.

=head1 COMMENTS/PLAIN TEXT
...
=head1 SUBCOMPONENTS/METHODS

=head2 E<lt>%def I<name>E<gt>

Each instance of this section creates a <i>subcomponent</i> embedded
inside the current component. Inside you may place almost anything
that a regular component contains -- embedded Perl, <%init> and <%args>
sections, etc.  The prohibited tags are <%def>, <%method>, <%once>,
and <%shared>.
...
Each instance of this section creates a <i>method</i> embedded inside
...
Use this section to assign arbitrary key/value attributes that
can be queried from other components.

    <%attr>
    color=>'blue'
    fonts=>[qw(arial geneva helvetica)]
    access=>sub { $r->connection->user eq 'joe' ? 1 : 0 }
    </%attr>

If the right-hand value is a subroutine reference (as in the third
line above), then that subroutine is invoked to determine the
value each time the attribute is queried.
...
The administrator may specify a set of default escape flags via
the L<Parser/default_escape_flags> Parser parameter. For example,
if the administrator specifies
...
Mason's C<$m-E<gt>cache> and C<$m-E<gt>cache_self> methods let components
...
Here's the typical usage of C<$m-E<gt>cache>:
...
The first C<$m-E<gt>cache> call attempts to retrieve this component's cache
...
cache by the second C<$m-E<gt>cache> call.

The default action for C<$m-E<gt>cache> is 'retrieve', so the first line can
be written as
...
C<$m-E<gt>cache> plus one additional argument, cache_file.  See the
...
C<$m-E<gt>cache_self> command that lets a component cache it's own
...
C<$m-E<gt>cache_self> is built on top of C<$m-E<gt>cache>, so it inherits all
the expiration options described earlier. C<$m-E<gt>cache_self> can also
...
Note to pre-0.8 users: the mc_suppress_http_header command is no longer needed.
For backwards compatibility, mc_suppress_http_header is still defined
but does nothing.

(Rationale: By the nature of HTTP headers mc_suppress_http_header was
only useful up until the first bytes of content were sent. Since Mason
now waits that long to send headers anyway, there is never any
justification for suppressing headers.)
...
primitives introduced in Mason 0.82.
...
problems. Future redesigns may well incorporate a deeper object
...
If one of these components has no primary section (no HTML or embedded
Perl), Mason automatically fills in an implicit
C<$m-E<gt>call_next>. This is useful for inherited components that
simply define methods and attributes without adding to the content.
...
page. First, use C<$m-E<gt>called_comp> to get a handle on the
appropriate component:

    my $self = $m->called_comp;
...
scopes. Variables declared, say, in the <%init> section of the main
component cannot be seen from methods or dynamic attributes.
...
number of standard methods and attributes for each page:
    ...
    <%method header>
    <h2><% $m->called_comp->attr('title') %></h2><p>
    </%method>
...
/products section of the site.
    ...
    <%attr>
    title=>'McGuffey Inc.: Products'
    </%attr>
    <%method header>
    ...
    </%method>

Because this autohandler has no primary section - no HTML or embedded
Perl - Mason automatically skips it when moving down the chain of
"wrapped" components. It exists solely to inherit methods and
attributes from.
