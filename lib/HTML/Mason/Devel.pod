...
o L<Initialization and Cleanup> (mainly C<< <%init> >>)
...
methods see B<L<HTML::Mason::Request>>.
...
C<< $m->current_comp >> and C<< $m->fetch_comp >>.
...
with the L<Interp/dhandler_name> Interp parameter.
...
with the L<Interp/autohandler_name> Interp parameter.
...
You can use L<Components that filter Content> if you want to filter
...
B<L<HTML::Mason::Request>> documentation for details.
...
L<Request/auto_send_headers> Request parameter available when running
