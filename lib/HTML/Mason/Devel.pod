...
but skips some options and features.
...
using the C<expire_if> option:

    my $result = $m->cache->get('key',
        expire_if=>sub { $_[0]->get_created_at < (stat($file))[9] });

C<expire_if> takes an anonymous subroutine, which is called with the
...
Mason offers a solution with the C<busy_lock> flag:
...
seconds.  In general, the duration passed to C<busy_lock> indicates
how long you're willing to allow this component to use an expired
cache value.
...
code will be incompatible with the new API. However, you can continue
to use the 1.0x cache API for a while by setting
L<data_cache_api|HTML::Mason::Request/data_cache_api> to '1.0'. All of
the $m-E<gt>cache options with the exception of C<tie_class> should be
supported.  The C<access_data_cache> function is no longer available,
however; this will need to be converted to use C<Cache::Cache>
directly, as described in the L<previous
section|HTML::Mason::Devel/Accessing a Cache Externally>.
