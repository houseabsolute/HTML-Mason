...
dhandlers entirely, with the L<Interp/dhandler_name> Interp
parameter.
...
C<mc_auto_comp> is also useful for calling the component manually,
e.g. if you want to suppress one or more original arguments.
...
to apply only to the current directory, use a conditional like this:
    ...
    <%init>
    return mc_auto_next if (mc_auto_comp =~ /\//);
    ... rest of autohandler ...
    </%init>
...
components, use any mc_ commands, or access $REQ from this section.
...
call a subcomponent simply use its name in <& &> or C<mc_comp>. A
...
busy_lock flag:
  ...
  my $result = mc_cache(action=>'retrieve',busy_lock=>'10sec',...);
...
the expiration options described earlier. B<mc_cache_self> can also
cache a component's return value; see the Commands manual for details.
...
mc_call_self (and C<E<lt>%filterE<gt>>, which is built on it) uses a bit of
...
Mason makes the powerful C<$r> Apache request object available as a
global in all components, granting access to a variety of server
internals, HTTP request data, and server API methods.

C<$r> is fully described in Apache.pod -- here is a sampling of
...
component you access the current request object via the global C<$REQ>.
...
call the component by using it inside <& &> or C<mc_comp>
