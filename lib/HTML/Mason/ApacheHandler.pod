...
Apache::Status (often http://localhost/perl-status/).  Default is
"HTML::Mason status".  This is useful if you create more then one
ApacheHandler object and want them all visible via Apache::Status.
...
=item head_optimization

True or undef, default is true. Indicates that once headers are sent
for a HEAD request, Mason should abort so as not to waste time
