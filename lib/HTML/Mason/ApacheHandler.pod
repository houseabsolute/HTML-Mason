...
HTML::Mason::ApacheHandler. The line must precede 'use HTML::Mason' to
take effect; otherwise HTML::Mason implicitly imports ApacheHandler
...
options are 'CGI' and 'mod_perl'; these indicate that a
CGI.pm or Apache::Request object (respectively) will be
created for the purposes of argument handling.

CGI is the default and comes with most Perl systems. You may wish to
switch to Apache::Request (available from CPAN) for speed and memory
efficiency.
