...
In scalar context, returns the next component in the content wrapping
chain; in list context, returns the remaining components in the
content wrapping chain. Usually called from an autohandler.  See the
