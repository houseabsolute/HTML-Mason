...
the specified stack level. e.g.
    ...
    my %top_args = @{$m->caller_args(-1)}    # get top-level arguments as a hash
...
Returns the next component in the content wrapping chain; usually
called from an autohandler.  See the L<Devel/autohandlers> section of
the I<Component Developer's Guide> for usage and examples.
...
the component, if (and only if) you are using the CGI module to do
this rather than the Apache::Request module.  The default is to use
the CGI module.  See the L<HTML::Mason::ApacheHandler> documentation
for more details.
