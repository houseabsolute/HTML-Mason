...
Generally called from a top-level component or dhandler. Aborts the
current request and restarts with the next applicable dhandler,
working up the tree. If no dhandler is available, returns NOT_FOUND.
...
Flushes the Mason output buffer. Under mod_perl, also calls $r->rflush
to flush the Apache buffer. Useful for making your servers seem more
responsive, especially for the initial bytes of output.
