...
the next section for a more refined strategy).  We use the PerlModule
...
Here is a configuration that enforces this naming scheme:
    ...
    <FilesMatch "(dhandler|autohandler|\.m(html|txt|pl))$"
      SetHandler perl-script
      PerlInitHandler Apache::Constants::NOT_FOUND
    </FilesMatch>

The first block causes real files named .html, .txt, or .pl to
be served through Mason.

The second block does the same thing for URLs that do not
correspond to real files. This is essential for using
L<HTML::Mason::Devel/dhandlers>.

The third block causes requests to private components to return 404
NOT_FOUND. This prevents unscrupulous users from even knowing which
private components exist.
