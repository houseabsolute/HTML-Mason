...
set any Mason configuration parameters, so Mason uses its default values.
...
=head2 Limiting URLs Handled by Mason
...
=head3 Securing top-level components
...
=head3 Securing top-level components with code
...
=head3 Controlling access with component attributes

If you wanted to control access to components based on component
attributes, as defined in an C<< <%attr> >> section, you might do
something like this:
