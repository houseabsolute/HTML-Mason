...
using the classes provide in the Mason distribution.  Subclassing is
...
set any Mason configuration parameters, so Mason provides uses its
default values.

=head2 Configuration Parameters

Since the default behavior will not work for everyone, Mason allows
you to flexibly configure its behavior via F<httpd.conf> configuration
parameters.
...
If you are familiar with Apache, you might have also thought of the
...
A second possibility is to indicate the extensions that should I<not>
be served through Mason, e.g.
...
This will send all extensions not listed in the C<LocationMatch>
directive through Mason. The disadvantage of this method is that you
have to explicitly list all of the non-Mason extensions used on your
site.
...
say F<private/>, that lies under the component root but outside the
document root. e.g.
...
outside access to components under F</usr/mason/comps/private>.
...
the Apache configuration directory.  It can be loaded like this:

  PerlRequire  handler.pl

The eg/ directory of the Mason distribution contains a couple sample
F<handler.pl> scripts.  Let's assume, that like the example, your
script defines a handler in the package C<MyApp::Mason>.  In this
case, your Apache configuration would look like this:
...
cause problems.  We want to decline requests for binary files while at
the same time, we want to open up the system to make Mason accept
directory requests.  Here's a bit of code that could be added to the
C<handler()> subroutine to do this:
...
just as easily via configuration directives in the F<httpd.conf> file.
...
default P<cache_class> to "NullCache".  This subclass faithfully
