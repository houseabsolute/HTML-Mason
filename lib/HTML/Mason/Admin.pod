...
=head2 Using global variables (new in 0.5)
...
warning. The solution is to register the global via the L<Parser/allow_globals>
parameter to Parser:

    my $parser = new HTML::Mason::Parser (..., allow_globals => [qw($dbh %session)]);
or
    $parser->allow_globals(qw($dbh %session))

Once a global is registered, Mason will silently include it in a C<use vars>
statement at the top of every component, thereby suppressing the
C<strict> warning.

If you are initializing the variable in handler.pl, you need to set it
in the package that components execute in, by default
B<HTML::Mason::Commands>:
...
Besides being a bit more pleasant on the eye, C<set_global> will do
...
standard DBM packages (B<SDBM>, B<ODBM>, B<NDBM>) are unsuitable for data
caching due to significant limitations on the size of keys and values.
Perl only comes with B<SDBM>, so you'll need to obtain a good-quality
package if you haven't already. At this time the best options are
Berkeley DB (B<DB_File>), available at www.sleepycat.com, and GNU's gdbm
(B<GDBM>), available at GNU mirror sites everywhere.

As far as the serialization methods, all of them should work fine.
B<Data::Dumper> is probably simplest: it comes with the latest versions
...
B<Storable> is significantly faster than the other options according to
