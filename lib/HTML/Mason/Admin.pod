...
roots in the spirit of Perl's @INC. To do so you must specify a list
of lists:
...
component calls C<$m->cache> or C<$m->cache_self> for the first time,
...
C<use_data_cache>=>0 to the Interp object. This will cause all $m->cache
...
The events are:
 ...
 CACHE_READ	attempt to read from		component path, cache key, success
		data cache ($m->cache)		flag (1 if item found, 0 otherwise)
...
translates to something like:
    ...
    my $_srctext = $m->file('/usr/local/www/htdocs/foo/bar');
    $m->comp('top');
