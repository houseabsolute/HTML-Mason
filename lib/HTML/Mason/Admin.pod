...
=head2 Configuration Names

When you configure Mason via your F<httpd.conf> file, Mason has to
create an ApacheHandler object for each request.  However, as a
performance enhancement, Mason caches previously created ApacheHandler
objects internally.  If you give your configurations a unique name,
then Mason can use this name as its cache key, and gain a significant
speed boost.

This name is set with the C<MasonConfigName> parameter.  An example
use of this would be the following:

  <Location /foo>
    PerlSetVar  MasonConfigName   Foo
    PerlSetVar  MasonAllowGlobals $Foo
    SetHandler  perl-script
    PerlHandler HTML::Mason::ApacheHandler
  </Location>

  <Location /bar>
    PerlSetVar  MasonConfigName   Bar
    PerlSetVar  MasonAllowGlobals $Bar
    SetHandler  perl-script
    PerlHandler HTML::Mason::ApacheHandler
  </Location>

If you don't provide configuration names, Mason will run substantially
slower.
...
space:
    ...
    <VirtualHost www.site1.com>
      ...
      <LocationMatch ...>
        PerlSetVar   MasonConfigName  Site1
        SetHandler   perl-script
        PerlHandler  HTML::Mason::ApacheHandler
      </LocationMatch>
    </VirtualHost>
    ...
    <VirtualHost www.site2.com>
      ...
      <LocationMatch ...>
        PerlSetVar   MasonConfigName  Site2
        SetHandler   perl-script
        PerlHandler  HTML::Mason::ApacheHandler
      </LocationMatch>
    </VirtualHost>
...
doing more work than necessary for each request.  This is done in two
different ways.

=over 4

=item 1. Cached by Name

As mentioned previously, if you set the C<MasonConfigName> parameter,
Mason will use this as its cache key.  This lets Mason find the right
ApacheHandler object very quickly.

=item 2. Cached by Configuration

The other cache, which is the slowest, is based on a key made up of
all your configuration values.  This is slowest because it requires
Mason to read your configuration I<before> it can do a cache lookup,
whereas lookups by an explicit name do not require reading the
