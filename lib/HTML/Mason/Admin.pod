...
the C<DocumentRoot> setting, and your Idata directory> will be a
directory called "mason" located under your server root.
...
You can, of course, manually specify these variables:
...
C<handler.pl> explicitly creates just one Mason objects, the
...
objects, including the Compiler, Lexer, Resolver, and Request.  These
objects, excluding the Request, are created once in the parent httpd
and then copied to each child process.
...
B<L<HTML::Mason::Request>>, B<L<HTML::Mason::Compiler::ToObject>>, and
B<L<HTML::Mason::Resolver::File>>. The B<L<HTML::Mason::Lexer>> takes
no parameters.
...
object files, using the L<Interp/make_component> Interp method.
