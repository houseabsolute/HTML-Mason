...
=head2 Declining requests for binary files

Mason should not be allowed to handle binary files (images, tarballs,
etc.) as regular components. Not only is there extra overhead, but
such a file may inadvertently contain a Mason character sequence such
as "<%".

The following line, placed at the top of the handler() routine,
...
=head2 Allowing requests for directories

By default Mason will decline requests for directories, allowing
Apache to serve up a directory index or a FORBIDDEN depending on your
configuration. Unfortunately this rule applies even if there is a
dhandler in the directory: /foo/bar/dhandler does not, by
default, get a chance to handle a request for /foo/bar.
...
A directory request can only be caught by a dhandler, in that
directory or above. The dhandler is responsible for setting the
content type.
...
headers. See L<Devel/sending_http_headers>.
