...
slowest requests. eg/perflog.pl in the Mason distribution is a log
...
Also, if you turn off the code cache temporarily, COMP_LOAD events
will tell you which components are loaded most often and thus good
...
overhead.  Do not expect your process growth to match the code cache
size exactly.
...
To turn off caching completely, set L<Interp/code_cache_max_size> to
0.
...
debugging and interpretation of compilation errors.  The
curious-minded can peek inside an object file to see exactly how a
component is converted into a Perl subroutine.
...
process, rather than loading them on demand, using C<interp-E<gt>preloads>.
Each child server will start with those components loaded in the
...
object files, using the Parser L<Parser/make_component> method.
...
In this example we assume each virtual server configuration section has a

    PerlSetVar site I<site_name>
