...
If you don't mind the performance implications of batch mode, then
running all servers (development and production) in batch mode offers
additional flexibility to developers:

=over

=item

HTTP headers can be manipulated in any component since they are not sent
til the end. See L<Devel/sending_http_headers>.

=item

$m->clear_buffer can help in presenting a clean error message to the
client. See L<Request/clear_buffer>.
