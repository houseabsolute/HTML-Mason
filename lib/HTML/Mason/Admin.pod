...
=head2 How Mason Creates and Caches ApacheHandler Objects

When you configure Mason under mod_perl via the Apache F<httpd.conf>
file, Mason must create one or more ApacheHandler objects internally
in order to handle requests.

Mason tries to aggressively cache these objects, in order to avoid
doing more work than necessary for each request.  There are three
levels of caching.

=over 4

=item 1. Cached at Startup

Mason first tries to create and cache an ApacheHandler object during
server startup.  It only does this if it guesses that you have just
one Mason configuration in your F<httpd.conf> file.

If Mason can read its configuration information during startup, I<and>
it finds a "MasonCompRoot" setting in that configuration, then it
assumes that you only have one Mason configuration defined.  Because
of the way Apache works, any configuration information inside an
enclosing block, such as a C<< <VirtualHost> >> or C<< <Location> >>
block, is not seen during server startup, and so will not be cached.
If you have Mason configured in the simplest way possible, which does
not require setting a component root, then Mason will assume you may
have multiple configurations.

=item 2. Cached by Location

If Mason wasn't able to create a single ApacheHandler object at
startup, then it assumes that you may have multiple Mason
configurations defined in different blocks of your F<httpd.conf> file.
When it makes a new ApacheHandler object, it will cache it based on
the value of C<< $r->location >> and C<< $r->server->server_hostname
>>.

The problem with this method is that only C<< <Location> >> C<<
<LocationMatch> >>, C<< <Directory> >>, and C<< <DirectoryMatch> >>
blocks cause C<< $r->location >> to return anything.  If your
configuration is directly inside a C<< <FilesMatch> >> or C<<
<VirtualHost> >> block, then C<< $r->location >> returns C<undef>, and
Mason cannot cache your configuration based on this.

=item 3. Cached by Configuration

The last cache, which is the slowest, is based on a key made up of all
your configuration values.  This is slowest because it requires Mason
to read your configuration I<before> it can do a cache lookup, whereas
the previous two methods do not require reading the configuration on
every request.
