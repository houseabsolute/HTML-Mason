...
The component root marks the top of your component hierarchy. By default,
it is the same as your Apache document root.
...
"mason" under your Apache server root.
...
One way is to specify a filename pattern in the Apache configuration,
e.g.:
...
Another way, if you are using a C<handler.pl> script, is to include
...
and text/plain, and declines others.
...
1. Set the L<decline_dirs|HTML::Mason::ApacheHandler/decline_dirs>
parameter to 0.
...
If you are using a C<handler.pl> script you can initialize the global there,
...
by default parses components in C<strict> mode, you'll need to invoke
C<use vars> to avoid a fatal globals warning.
...
Alternatively, you can initialize the global in the E<lt>%onceE<gt> or
E<lt>%initE<gt> section of a top-level C<autohandler>:
...
or so of code in your C<handler.pl>, you can integrate Jeffrey Baker's
C<Apache::Session> into your application and make a tied global
session variable available to all components.

The Mason Sessions How-To, at ..., is the best source of information
about this surprisingly tricky subject.
...
L<data_cache_defaults|HTML::Mason::Interp/data_cache_defaults>
...
The maximum size of the cache is specified with the L<Interp's
code_cache_max_size|HTML::Mason::Interp/code_cache_max_size>
...
To turn off code caching completely, set L<Interp's
code_cache_max_size|HTML::Mason::Interp/code_cache_max_size> to 0.
...
the L<Interp's use_object_files|HTML::Mason::Interp/use_object_files>
parameter to 0.

=head2 Preloading

You can tell Mason to preload a set of components in the parent
process, rather than loading them on demand, using the L<Interp's
preloads|HTML::Mason::Interp/preloads> parameter.  Each child server
...
L<static_source|HTML::Mason::Interp/static_source> mode. In this mode
...
L<error_mode|HTML::Mason::Request/error_mode> to "output" or "fatal"
respectively.
...
the result to standard output:
    ...
    $interp->exec(<path-to-component>, <args>...);

Because no component root was specified, the root is set to '/'
(meaning that any file may be used as a component) and the initial
component may be relative to the current working directory. This is
most useful for an isolated component or a few components in a single
directory. For more complex applications with a full component tree,
you will probably want to specify a component root.

It should be noted that relative component paths are resolved based on
the current working directory I<when the Interpreter was created>.
Subsequent changes to the current working directory will be ignored.
