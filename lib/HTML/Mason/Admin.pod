...
=head1 GLOBAL SETTINGS FILE

Mason/Config.pm contains global settings for your system, such as the
presence or absence of certain optional modules.

Makefile.PL initially creates Config.pm based on your environment,
...
The next three sections discuss ways to configure a Mason site:
...
* Configuration via handler.pl. Most complex but most flexible.

We recommend that you start with the simplest method and work your
way forward as the need for flexibility arises.

=head1 MINIMAL CONFIGURATION

Directives must be added to httpd.conf or srm.conf to specify which
requests to handle through Mason. A simple configuration looks like:
    ...
    <Location />
        ...
        MasonDataDir "/usr/local/www/mason"
    </Location>

The I<component root> (specified by MasonCompRoot) is the top of your
tree of component source files. In simple configurations the component
root is the same as the server's DocumentRoot.  More complex
configurations may specify several different DocumentRoots under a
single component root.

When Mason handles a request, the request filename
(C<$r-E<gt>filename>) must be underneath your component root -- that
way Mason has a legitimate component to start with.  If the filename
is not underneath the component root, Mason will place a warning in
the error logs and return 404.

The I<data directory> (specified by MasonDataDir) is a writable
directory that Mason uses to implement various features and
...
Mason treats them in different ways.  The different parameter types
are:
...
necessary if the string contains spaces.
...
The string is treated as a piece of code.  This is used for parameters
that expect code references.  For example, an anonymous subroutine
might look like:

 PerlSetVar  MasonDieHandler  "sub { die join ' -- ', @_ }"

A named subroutine call must be wrapped in an anonymous subroutine:

 PerlSetVar  MasonDieHandler  "sub { Carp::croak(@_) }"

=item * list

To set a list parameter, use C<PerlSetVar> for the first value and
then C<PerlAddVar> for all subsequent values, like this:

 PerlSetVar  MasonPreloads  /foo/bar/baz.comp
 PerlAddVar  MasonPreloads  /foo/bar/quux.comp

=back

Almost all of Mason's configuration parameters correspond to new()
parameters for the B<L<HTML::Mason::Parser>>,
...
values such as:
 ...
 PerlSetVar  MasonCompRoot  "root1 => /foo/comp/root1"
...
=item * MasonDataDir - string (required)
...
=item * MasonCurrentTime - string

Corresponds to the C<current_time> parameter.

=item * MasonDataCacheDir - string

Corresponds to the C<data_cache_dir> parameter.

=item * MasonDieHandler - code or 0

Corresponds to the C<die_handler> parameter.
...
=item * MasonOutMode - string

Corresponds to the C<out_mode> parameter.

=item * MasonPreloads - list

Corresponds to the C<preloads> parameter.

=item * MasonStaticFileRoot - string

Corresponds to the C<static_file_root> parameter.
...
=item * MasonUseDataCache - boolean

Corresponds to the C<use_data_cache> parameter.
...
=item * MasonVerboseCompileError - boolean

Corresponds to the C<verbose_compile_error> parameter.
...
These are the only parameters that do I<not> correspond to new() parameters.

=over 4

=item * MasonArgsMethod - string

Valid values for this parameter are C<CGI> or C<mod_perl>.  This
determines whether to use the C<CGI> or C<Apache::Request> module for
argument handling.  Please see L<HTML::Mason::ApacheHandler/PARAMETERS TO THE use() DECLARATION>
for more details.

This can only be set once, regardless of how you set
MasonMultipleConfig.

Defaults to C<CGI>.

=item * MasonMultipleConfig - boolean

If this parameter is true, Mason assumes that you may have multiple
sets of configuration directives defined, perhaps for multiple
locations or virtual hosts.  This means that Mason will create a new
set of parser/interp/handler objects for every request.  If this
parameter is false, Mason simply creates a single set at server
startup to maximize shared memory. Defaults to false.

If you set this option, the Apache child processes will need
the proper permissions to create the data directories.

=back

=head1 CONFIGURING MASON VIA A HANDLER SCRIPT
...
and examples we call this script handler.pl, though you may name it
whatever you like.

The handler.pl file is responsible for creating the three Mason
...
well-placed Perl in your handler.pl.  In this section we present the
basics of setting up handler.pl as well as some ideas for more
...
Ordinarily, when C<$m-E<gt>cache> is called, Mason passes to the cache
constructor the C<namespace>, C<username>, and C<cache_root>
options, along with any other options given in the C<$m-E<gt>cache>
method.
...
The options passed to individual C<$m-E<gt>cache> calls override these
defaults.

=item Disabling data caching

If for some reason you want to disable data caching entirely, set
C<Interp/use_data_cache> to false. This causes all C<$m-E<gt>cache>
calls to return a C<Cache::NullCache> object, which correctly
implements the API but never stores data.
...
object files, using the L<Parser/make_component> Parser method.
...
headers (see L<Devel/sending_http_headers>) and in handling
mid-request error conditions (see L<Request/clear_buffer>).
...
These examples extend the L<Mason/single site configuration>
