...
it, the fully qualified component path is available from C<mc_auto_comp>.
For example, here we base the page title partly on the component name:
    ...
    my $section = ($comp =~ /mktg.html$/ ? 'Marketing' : ...);
    </%init>
...
Autohandlers apply only to the current directory, not to its
subdirectories.  Although it was very tempting to implement a single
autohandler applying to an entire component hierarchy (e.g. for a
site-wide template), there were several drawbacks.  The first is
performance: Mason would have to scan not only the current directory
but all parent directories for a file named "autohandler"; this would
slow down each request even where autohandlers were not being
used. The second is readability: someone trying to determine how a
page is rendered would have to scan through parent directories in the
right order. That said, we will continue to seek out a reasonable
solution for this feature.

For now, if you want to use the same autohandler in multiple
subdirectories, there are several alternatives to simply copying
it. One is to create symbolic links pointing back to the master
autohandler (assuming your OS supports this); Mason seems to do fine
with symbolic link components.  Another is to create minimal
autohandlers of the form:

    <%init>
    mc_comp('/foo/bar/autohandler',%ARGS');
    </%init>
...
application.  The main differences to keep in mind:

o dhandlers are invoked only when no appropriate component exists;
autohandlers are invoked only in conjunction with a matching
component.

o dhandlers can handle any request underneath its directory hierarchy;
autohandlers will only handle a request in the same directory.

As a rule of thumb: use an autohandler when you have a set of
components to handle your pages and you simply want to augment them
...
example, you might have a mix of virtual and real URLs to which you
