...
called more or less like subroutines, using the <& &> tag.
Top-level components must reside within the server's
...
that file as a component.  It might in turn call other components and
execute some Perl code, or it might be nothing more than static HTML.
...
thereby surprising the C<if> statement! Conversely, anything returned
by C</shared/masthead> above would be ignored by the <& &> tag.
...
In fact, internally Mason just treats an HTTP request as a first
component call! In this case, assuming the component and document
roots are the same:

    <& /mktg/prods.html, str=>'dog', lst=>[2,3,4] &>
...
that uses the component. However, if the component is preloaded, it
...
Turns off processing of Mason syntax. Useful, for example, when
