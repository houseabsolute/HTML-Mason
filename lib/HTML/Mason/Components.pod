...
run-time. %ARGS can be used whether or not you have a <%perl_args>
section.
...
At the beginning of a request, Mason calls almost every one of the
...
the next Mason release.  Conversely, when FakeApache is updated, it
may cease to work with older versions of mod_perl.
