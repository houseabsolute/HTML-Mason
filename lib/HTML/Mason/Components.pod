...
sections you can embed in your components. By "in-line" I mean these
sections are generally embedded within HTML and execute in the order
they appear. Other, specialized Perl sections are tied to component
events like initialization and cleanup, argument definition, etc.
Those are covered later in I<L<Other Perl Sections>>.
...
I've used bad form here for the sake of example; the leading '%' line
could simply have been placed into the multi-line <%perl> section.
...
caching, file includes, and so on. The majority of Mason commands are
for advanced users, but a few (like C<mc_comp()>, for calling other
components) see widespread use. B<L<HTML::Mason::Commands>> is the
reference for all Mason commands.

=head1 HOW COMPONENTS ARE INVOKED

Components are invoked in two ways: top-level components respond
directly to HTTP requests, while other, subordinate components are
called more or less like subroutines, using Mason's B<mc_comp>
command. Top-level components must reside within the server's
DocumentRoot, while other components live within Mason's component
root (webmasters sometimes equate the two).

=head2 Top-level Components

A top-level component is simply one that's called from an URL, such
as:

    http://www.foo.com/mktg/prods.html

Here, C<prods.html> may be nothing more than a static HTML file, since
components don't I<have> to contain any Mason stuff.
...
This component must now contain some Mason elements in order to read
the query arguments and do something interesting with them.  To read
the arguments, prods.html should contain a C<E<lt>%perl_args%E<gt>>
section (see I<L<Parameter Passing>> below) describing the input
variables and their data types, in this case a scalar (C<$str>) and a
three-element list (C<@lst>). Thus components are relieved of the
form-parsing chores common to CGI scripts.

A catch-all %ARGS hash is also available for components that wish to
deal with HTTP request arguments directly. %ARGS is defined
automatically; components need not list it in a
C<E<lt>%perl_args%E<gt>> section.

These arguments are subsequently available to the component as
regular Perl variables.
...
subdirectory C<LocalNews> nor the component C<locStory1.html>:
...
C<$r-E<gt>path_info> and use it as a retrieval key. Optionally, the
Mason command C<mc_dhandler_arg()> returns the same path_info stripped
of the leading slash ("LocalNews/locStory1.html"). This is sometimes
more useful that the absolute path returned by C<$r-E<gt>path_info>.

=head2 Components as Subroutines

Components typically build themselves up from other components--this
is in fact the keystone of Mason's component-based approach. Good
components therefore resemble good subroutines in that they're written
for reuse.

To call one component from another, use Mason's B<mc_comp> command:

B<mc_comp (compPath, name=>>B<value, ...[, STORE=>>B<ref ])>
...
variable instead of standard output (programmers: think sprintf vs.
printf). For example:
...
list and hash must be passed as references, but in /mktg/prods.html
they are automatically dereferenced into a local list and hash. While
convenient, this means that @b and %c are copies--changing them does
not modify the caller's version. To pass a reference, declare it as a
scalar value.

Unlike Perl subroutines, Mason components I<do not> read their
parameters from Perl's @_ array. Rather, Mason imposes its own typed
argument passing protocol in which components define a
C<E<lt>%perl_args%E<gt>> section listing parameter names, types, and
...
=head2 Return Values and Context

Components generally fall into one of two categories: HTML
generators, and functions that compute a value. The latter behave
like normal Perl functions, while HTML generators (far more common),
rarely return values--in the name of performance they stream their
output immediately to the browser.

Mason in fact adds a C<return undef> to the bottom of each component
to provide an empty default return value. For HTML-generating
components, this allows the convenient idiom:

    <% mc_comp('foo') %>

which, if you think about it, actually prints two things: foo
internally prints some HTML, while C<E<lt>%mc_comp('foo')%E<gt>>
prints the return value of foo (C<undef>).

To return your own value from a component, you I<must> use an
explicit C<return> statement. In this case the component behaves like
a normal Perl subroutine with regard to return values and scalar/list
context:
...
The Perl sections we're about to describe are more specialized than
the in-line sections described above. Several are tied to phases of
the component execution sequence, which goes something like this:

    1. Initialize arguments in HTML::Mason::Commands package
    2. <%perl_init%> section
...
Technically a <%perl_init> block is equivalent to a <%perl> block at
...
Since C<E<lt>%perl_initE<gt>> sections fire before any HTTP headers
are sent, they should do their work quickly to prevent dead time on
the browser side.

=item <%perl_cleanup> xxx </%perl_cleanup>

Useful for cleanup code. For example: closing a database connection or
closing a file handle.

Technically a <%perl_cleanup> block is equivalent to a <%perl> block at
...
the end of blocks, <%perl_cleanup> sections are rarely needed.

=item <%perl_args> xxx </%perl_args>
...
Useful for suppressing unwanted newlines before Perl lines and block
tags. In HTML components, this is mostly useful for fixed width areas
like <PRE> tags, since browsers ignore white space for the most
part. An example:
...
=item <%perl_doc> xxx </%perl_doc>

Most useful for a component's main documentation.  One can easily
write a program to sift through a set of components and pull out their
...
Turns off processing of Perl sections; useful when documenting Mason
...
</%perl_off> itself! When all else fails, use C<mc_out()>:

    %mc_out('The tags are <%perl_off> and </%perl_off>.');

=back

=head1 EXAMPLES AND RECOMMENDED USAGE
...
=item HTML list formed from array
...
=item HTML list formed from hash
...
=item HTML table formed from list of hashes
...
used right at the top of a C<E<lt>%perl_init%E<gt>> section:

    ... <primary section, body of page> ...
    <%perl_init>
...
=head1 USING THE PERL PROFILER (new in 0.4)
...
Do not declare named subroutines within your components. Mason wraps
your subroutine within its own C<sub {..}>, which creates a
subroutine within a subroutine, which makes Perl very unhappy.
Instead, use local anonymous subroutines or create another component
and call with B<mc_comp>.
...
=item Separating Perl From HTML
...
Components in samples/:
    -- Chris: generic site kit
    -- Chris: madlib
    -- Chris: slide show maker (use to present descriptive slides other for
            samples!)
    -- bullet expander (convert to component first)


COMPONENTS ARE SUBROUTINES

The subroutine analogy mentioned above has a technical basis. Mason
actually "compiles" components to Perl subroutines, basically by adding
print statements to non-Perl text and wrapping a sub {} around the
whole thing.  For example, the compiled form of the hello world
example from above looks something like:
...
This produces a reference to a subroutine that, when invoked, executes
the component. The code shown here is greatly simplified; see [[Link to
full compilation explanation and example.]]

Probably the main reason I chose this representation is laziness - the
parser is small and simple and takes optimal advantage of Perl's own
parser. But components enjoy other advantages of their subroutine roots:

Components can take arguments. For example, the left TOC component
above might take as arguments a background color and a set of links to
highlight, while establishing reasonable defaults for typical calls.
Rather than depending on @_, Mason allows you to declare named arguments
in a special args section. [[Link to section about arguments.]]
